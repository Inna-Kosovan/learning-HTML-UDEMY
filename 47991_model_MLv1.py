...
class TimeSeriesForecasting:
    ...
    def data_quality_check(self):
        ...
        # Initialized the missing values columns list
    ...
    def generate_future_data(self, timestamps):
        ...
        if 'EVENT_TIME' in future_data.columns:
            future_data['EVENT_TIME'] = pd.to_datetime(
                future_data['EVENT_TIME'])
            future_data.set_index('EVENT_TIME', inplace=True)
            for time_feature in ['year', 'month', 'day', 'hour', 'minute', 'second']:
                future_data[f'EVENT_TIME_{time_feature}'] = getattr(
                    future_data.index, time_feature)
        return future_data
    ...
    def preprocess_data(self):
        ...
        if 'EVENT_TIME' in self.df.columns:
            self.df['EVENT_TIME'] = pd.to_datetime(
                self.df['EVENT_TIME'], errors='coerce')
    ...
    def train_models(self, target_col, model_types=['GradientBoosting', 'RandomForest', 'XGBoost']):
        ...
        X_train, y_train = self.train.drop(
            columns=target_col), self.train[target_col]
        X_test, y_test = self.val.drop(
            columns=target_col), self.val[target_col]

        for model_type in model_types:
            self._hyperparameter_tuning_and_training(
                target_col, X_train, y_train, X_test, y_test, model_type)
    ...
    def _hyperparameter_tuning_and_training(self, target_col, X_train, y_train, X_test, y_test, model_type='GradientBoosting'):
        ...
        grid_search = GridSearchCV(
            model, param_grid, cv=KFold(n_splits=5), n_jobs=-1)
        ...
        cv_scores = cross_val_score(
            best_model, X_test, y_test, cv=KFold(n_splits=5))
        print(f'Cross-validation scores for {target_col}: {cv_scores}')
        print(
            f'Mean cross-validation score for {target_col}: {np.mean(cv_scores)}')
        importance = best_model.feature_importances_
        importance_dict = dict(zip(X_train.columns, importance))
        sorted_importance = sorted(
            importance_dict.items(), key=lambda x: x[1], reverse=True)
        print(
            f"Top 5 important features for {target_col}: {sorted_importance[:5]}")
        self.log_execution_time(
            start_time, "Hyperparameter tuning and training completed in")
    ...
    def _get_model_and_param_grid(self, model_type):
        if model_type == 'GradientBoosting':
            ...
            raise ValueError(
                "Invalid model_type. Choose 'GradientBoosting', 'RandomForest' or 'XGBoost'")
    ...
    def load_models(self):
        for target_col in self.models.keys():
            self.models[target_col] = joblib.load(
                f"{self.save_directory}/{target_col}_model.pkl")
...
def main():
    ...
    data = pd.read_csv(
        "/data/works/codes/neiron4_model_ML/TRADE_1H/47991_DataProcessor_v3.csv")
