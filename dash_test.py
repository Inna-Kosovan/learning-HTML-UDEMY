...
def read_and_process_file(uploaded_file):
    if uploaded_file:
        try:
            # Specify the date format when reading the 'EVENT_TIME' column
...
def create_combined_chart(df1, column1, currency1, df2, column2, currency2):
    ...
    chart = alt.Chart(combined_df).mark_line().encode(
        ...
        color=alt.Color('Category:N', scale=alt.Scale(domain=[df1['Category'].iloc[0] if not df1['Category'].empty else None,
                                                      df2['Category'].iloc[0] if not df2['Category'].empty else None],
                                              range=['yellow', 'red'])),
    ...
    if df1 is not None and not df1.empty and df2 is not None and not df2.empty:
        ...
        histogram_chart = alt.Chart(combined_df).mark_bar().encode(
            x=alt.X('utchoursminutesseconds(EVENT_TIME):T', title='Event Time',
                    axis=alt.Axis(format='%H:%M:%S', labelFontSize=15, labelColor='white', labelAngle=-90)),
            y=alt.Y('Value:Q', title='Values', scale=alt.Scale(zero=False)),
            color=alt.Color('Category:N', scale=alt.Scale(domain=combined_df['Category'].unique().tolist(), range=['yellow', 'red'])),
            tooltip=['EVENT_TIME:T', 'Value:Q', 'Category:N']
        ).properties(
            title=f"{currency1} {column1} and {currency2} {column2} Over Time",
            background='black',
...
def generate_file_processing_ui(uploaded_file, file_label, key_suffix):
    try:
        ...
        if selected_column and selected_column in time_filtered_df.columns:
            ...
            return df, selected_currency, selected_column, selected_start_time, selected_end_time

        else:
            st.warning(f"Please upload the {file_label} CSV file.")

    except Exception as e:
        st.error(f"An error occurred while processing the {file_label} file: {e}")
        # Log the traceback for more detailed information
        import traceback
        traceback.print_exc()

        # Return an empty DataFrame and other variables to avoid 'None'
